...
const getLineageGraphNodes = (rawResponse: LineageResponseV1): Node[] => {
    return [
        ...getDatasetContainerNodes(rawResponse),
        ...Object.values(rawResponse.nodes.jobs).map((job) =>
            getJobNodeForLineage(job, rawResponse),
        ),
    ];
};
export default getLineageGraphNodes;
